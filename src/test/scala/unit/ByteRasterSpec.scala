...
object ByteRasterSpec extends Specification {
  "Byte raster" should {
    "apply correct value" in new rasterScope {
      val res = raster(0,1)

      (res must beAnInstanceOf[Byte]) and (res must beEqualTo(2))
    }
    ...
    "sum to a byte raster" in new rasterScope {
      ...
      (res must beAnInstanceOf[Raster[Byte]]) and (res(1,1) must beEqualTo(8))
    }
    ...
    "mutiply to a float raster" in new rasterScope {
      val floatRaster = Raster(Array(1,2,3,4), 2, 2).map(_.toFloat)
      val res = raster * floatRaster
    ...
    "sum to a byte constant" in new rasterScope {
      ...
      (res must beAnInstanceOf[Raster[Byte]]) and (res(0,1) must beEqualTo(4))
    }
    ...
    "or to a byte constant" in new rasterScope {
      ...
      (res must beAnInstanceOf[Raster[Byte]]) and (res(1,0) must beEqualTo(3))
    }
    ...
    "negate" in new rasterScope {
      ...
      (res must beAnInstanceOf[Raster[Byte]]) and (res(0,0) must beEqualTo(-1))
    }
  ...
  "Byte constant" should {
    "sum to a byte raster" in new rasterScope {
      val res = 2 + raster

      (res must beAnInstanceOf[Raster[Byte]]) and (res(0,1) must beEqualTo(4.0))
    }
  ...
  trait rasterScope extends Scope {
    lazy val raster = Raster(Array(1,2,3,4), 2, 2).map(_.toByte)
    lazy val const = 2
  }
}